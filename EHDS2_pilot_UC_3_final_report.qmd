...
filename <- "EHDS2_pilot_UC1_data_agg_mockup_%s.csv"
...
## Mapping postcode to LAU2 code - specific for Belgium Node
# https://public.opendatasoft.com/explore/dataset/georef-belgium-postal-codes/export/?location=10,50.7908,3.79784&basemap=jawg.light
# lau2_shape <- st_read("georef-belgium-postal-codes.shp")
# 
# lau2_shape <- lau2_shape %>% 
...
# 
# df <- df %>%
#   left_join(lau2_shape %>% st_drop_geometry, join_by(residence_area_lau_cd == POSTCODE)) %>%
#   mutate(residence_area_lau_cd = NUTS_ID) %>%
#   select(-NUTS_ID)
# 
    ...
    map <- map %>%
      addPolygons(data = dataRQ, 
                  ...
                  label = ~paste(
                    ...
                    "n: ", n, "<br/>",
                    title, ":<span style='color: black; font-weight: bold;'>", round(get(variable), 2), "%</span><br/>",
                    "</div>"
      ...
      map <- map %>%
        addPolygons(data = filtered_data,
                    ...
                    label = ~paste(
                      ...
                      "Name: <span style='color: green; font-weight: bold;'>", NUTS_NAME, "</span><br/>",
                      "n: ", n, "<br/>",
                      group, ":<span style='color: black; font-weight: bold;'>", round(get(variable), 2), "%</span><br/>",
                      "</div>"
                    ) %>% lapply(htmltools::HTML),
                    group = group)
    }
...
rq1_2_gender_tests <- rq1_2_gender_tests %>%
  mutate(n = as.character(n)) %>%
  mutate(n = ifelse(is.na(n), "Excluded", n))
...
rq3_gender_doses <- rq3_gender_doses %>%
  mutate(n = as.character(n)) %>%
  mutate(n = ifelse(is.na(n), "Excluded", n))
