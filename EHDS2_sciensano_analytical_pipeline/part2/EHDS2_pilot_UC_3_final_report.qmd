...
extract_group_summary <- function(rq_df, rq_label, cohort_label) {
  ...
  data.frame(
    ...
    sd_test_positives_nm = extracted_values_group[14],
    iqr_test_positives_nm = extracted_values_group[15],
    ...
    sd_dose_nm = extracted_values_group[19],
    iqr_dose_nm = extracted_values_group[20],
    ...
    # Vaccinated population hospitalization
    hospitalised_true_vaccinated = extracted_values_group[25],
    hospitalised_true_vaccinated_rate = extracted_values_group[26],
...
colnames(global_metrics_df) <- c(
  ...
  # Vaccinated population hospitalization
  "Vaccinated Hospitalised",  # hospitalised_true_vaccinated
  "Vaccinated Hosp. Rate (%)" # hospitalised_true_vaccinated_rate
...
# Combine columns (mean ± SD) and (median (IQR)), and remove original columns
format_and_cleanup_columns <- function(df) {
  # Combine Avg Age and Age SD
  df <- df %>%
    mutate(`Avg Age (± SD)` = paste0(`Avg Age`, " (± ", `Age SD`, ")"),
           `Median Age (IQR)` = paste0(`Median Age`, " (", `Age IQR`, ")")) %>%
    select(-`Avg Age`, -`Age SD`, -`Median Age`, -`Age IQR`)
  
  # Combine Avg Tests and Tests SD
  df <- df %>%
    mutate(`Avg Tests (± SD)` = paste0(`Avg Tests`, " (± ", `Tests SD`, ")"),
           `Median Tests (IQR)` = paste0(`Median Tests`, " (", `Tests IQR`, ")")) %>%
    select(-`Avg Tests`, -`Tests SD`, -`Median Tests`, -`Tests IQR`)
  
  # Combine Avg Doses and Doses SD
  df <- df %>%
    mutate(`Avg Doses (± SD)` = paste0(`Avg Doses`, " (± ", `Doses SD`, ")"),
           `Median Doses (IQR)` = paste0(`Median Doses`, " (", `Doses IQR`, ")")) %>%
    select(-`Avg Doses`, -`Doses SD`, -`Median Doses`, -`Doses IQR`)
  
  # Combine Avg Positive Tests and Positive Tests SD
  df <- df %>%
    mutate(`Avg Positive Tests (± SD)` = paste0(`Avg Positive Tests`, " (± ", `Positive Tests SD`, ")"),
           `Median Positive Tests (IQR)` = paste0(`Median Positive Tests`, " (", `Positive Tests IQR`, ")")) %>%
    select(-`Avg Positive Tests`, -`Positive Tests SD`, -`Median Positive Tests`, -`Positive Tests IQR`)
  
  return(df)
}

# Apply the formatting and cleanup
formatted_global_metrics_df <- format_and_cleanup_columns(global_metrics_df)

write_xlsx(formatted_global_metrics_df, "global_metrics.xlsx")
